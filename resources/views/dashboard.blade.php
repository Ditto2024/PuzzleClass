            ...
            <div class="bg-white rounded-[24px] p-5 shadow-sm">
                ...
                @if($canClaimDailyReward)
                    <form method="POST" action="{{ route('daily-reward.claim') }}">
                        @csrf
                        <button class="mt-4 bg-emerald-400 text-white px-5 py-2 rounded-full font-bold">
                            Claim
                        </button>
                    </form>
                @else
                    <button disabled class="mt-4 bg-gray-300 text-white px-5 py-2 rounded-full font-bold">
                        Claimed
                    </button>
                @endif
            </div>
            ...
            <div class="mt-5 bg-white rounded-[24px] p-5 shadow-sm">
                ...
                <div class="flex items-center justify-between mt-1">
                    <h3 class="text-2xl font-black">
                        1 Puzzle • {{ $quests->first()->reward_points }} Poin
                    </h3>
